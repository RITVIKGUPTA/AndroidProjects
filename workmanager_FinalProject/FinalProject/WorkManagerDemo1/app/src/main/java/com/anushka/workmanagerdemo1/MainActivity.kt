...
class MainActivity : AppCompatActivity() {
    ...
    private fun setPeriodicWorkRequest(){
        val listen : ListenableFuture<String>
        val workManager = WorkManager.getInstance((application))
       val periodicWorkRequest = PeriodicWorkRequest
           .Builder(DownloadingWorker::class.java,16,TimeUnit.MINUTES)
           .build()
      workManager.enqueue(periodicWorkRequest)
       val a =  workManager.getWorkInfoById(periodicWorkRequest.id)
        Log.i("MainActivity", "${a::class.java}")
        workManager.getWorkInfoByIdLiveData(periodicWorkRequest.id).observe(this, Observer {
            textView.text = it.state.name
            Log.i("MainActivity", "${it.state.name} ${it.state.isFinished}")
            if(it.state.isFinished){
                Toast.makeText(application, "downloading work is finished", Toast.LENGTH_SHORT).show()
            }
        })
